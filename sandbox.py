...
import time
import os
from financial_model import get_financial_filter_ctes, CHICAGO_NEW_BUILD_RENTS, DEFAULT_NEW_BUILD_RENT
...
def run_sandbox():
    ...
    t0 = time.time()
    print("⏳ [1/4] Isolating parcels for the 4 test neighborhoods...", end="", flush=True)
    con.execute("""
    ...
    con.execute("""
        ...
        u_agg AS (SELECT SUBSTR(LPAD(CAST(pin AS VARCHAR), 14, '0'), 1, 10) as pin10, ANY_VALUE(CAST("class" AS VARCHAR)) as property_class FROM assessor_universe GROUP BY 1),
        ...
        SELECT bp.pin10, bp.geom_3435, bp.neighborhood_name, bp.area_sqft, bp.zone_class,
            ...
            CASE 
                ...
                ELSE 1.0 END as existing_units,
    ...
    t0 = time.time()
    print("⏳ [3/4] Calculating Transit Distances (Using fast ST_Buffer indexes)...", end="", flush=True)
    con.execute("""
        ...
        -- 🔥 THE OPTIMIZATION: ST_Intersects(ST_Buffer()) engages the R-Tree Index and avoids Cross Joins
    ...
    t0 = time.time()
    print("⏳ [4/4] Executing Real Estate Pro Forma equations...", end="", flush=True)
    financial_ctes = get_financial_filter_ctes(source_table_name="parcel_calculations")

    query = f"""
        WITH parcel_calculations AS (
            SELECT pd.geom_3435 as center_geom, pd.neighborhood_name, pd.area_sqft, pd.zone_class, 1 as parcels_combined, pd.existing_units as tot_existing_units, pd.property_class as primary_prop_class, pd.tot_bldg_value, pd.tot_land_value, pd.building_age, pd.existing_sqft, pd.prop_address,
